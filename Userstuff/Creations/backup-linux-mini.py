...
def mkfile(a):
    if not os.path.exists(a):
        ...
        print(f"file {a} created")
    elif os.path.isfile(a):
        x = input("file exists, overwrite?[y/n]: ")
        if x.lower().startswith("y"):
            with open(a, "w") as f: pass
            print(f"file {a} overwritten")
        else:
            print("cancelled")
...
def cd(folder_name):
    ...
    # 5. If multiple matches, list and choose
    print("Found the following folders:")
...
def find(name, path="."):
    ...
    # 1️⃣ Search current folder recursively
    for root, dirs, files in os.walk(path):
        try:
            if name in files:
    ...
    # 2️⃣ If not found, ask to search entire PC
    if not found_items:
    ...
    # 3️⃣ After ALL searching, check results
    if found_items:
        ...
        for root, dirs, files in os.walk(path):
            try:
                if name in files:
                    ...
                    found = True
                if name in dirs:
                    print("Found folder:", os.path.join(root, name))
                    found = True
            except PermissionError:
                continue
